...
std::map<vector<float>, int> word_vectors_target;
int kSOS;
int kSOS2;
int kEOS;
int kEOS2;
...
struct EncoderDecoder {
	...
	r2lbuilder(IN_LAYERS, WORD_VECTOR_DIM, HIDDEN_DIM, &model), outbuilder(OUT_LAYERS, WORD_VECTOR_DIM, HIDDEN_DIM, &model) {
		p_c = model.add_lookup_parameters(VOCAB_SIZE, {WORD_VECTOR_DIM});
		...
		p_outbias = model.add_parameters({VOCAB_SIZE});
                std::vector<Tensor>* vals = p_c.values();
	...
	model.set_updated_lookup_param(&p_c, false);
	}
	...
	if (isTrain) {
		...
		string line;
                ...
                }
                {
                  ...
                  assert(vects.size() == WORD_VECTOR_DIM);
                  word_vectors_target[vects] = word;
                }
